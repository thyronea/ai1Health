...
<div class="modal fade" id="administer_kinrix" tabindex="-1" aria-labelledby="administer_kinrixLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header text-center">
        <h1 class="modal-title w-100 fs-5" id="administer_kinrixLabel">Administer Kinrix</h1>
      ...
      <div class="modal-body">
        <div class="col-md-12">
          <form class="" action="" method="post">
            ...
            <input type="hidden" class="form-control form-control-sm mt-2" name="uniqueID" id="kinrix_uniqueID" required>
            <input type="hidden" class="form-control form-control-sm mt-2" name="type" value="KINRIX" required>
            <input type="hidden" class="form-control form-control-sm mt-2" name="combo_type" value="KINRIX" required>
            ...
            <select id="kinrix_ID" name="id" class="form-select form-select-sm mb-2" onchange="add_kinrix()" required>
                  <option></option>
                  <option disabled>Select from inventory</option>
                  <?php
                    $groupID = mysqli_real_escape_string($con, $_SESSION['groupID']); $sql = "SELECT * FROM inventory WHERE groupID='$groupID' AND name='DTaP/IPV - kinrix Single Dose Vials' ";
                    $sql_run = mysqli_query($con, $sql);
                    $kinrix_SDV = mysqli_num_rows($sql_run);
                    while ($kinrix_SDV = mysqli_fetch_array($sql_run))
                    {
                      echo "<option value='". htmlspecialchars($kinrix_SDV['id']) ."'>" .htmlspecialchars($kinrix_SDV['name']) .' ' .'('.htmlspecialchars($kinrix_SDV['funding_source']).')' ."</option>" ;
                    }

                    $groupID = mysqli_real_escape_string($con, $_SESSION['groupID']); $sql = "SELECT * FROM inventory WHERE groupID='$groupID' AND name='DTaP/IPV - kinrix Single Dose Syringes' ";
                    $sql_run = mysqli_query($con, $sql);
                    $kinrix_SDS = mysqli_num_rows($sql_run);
                    while ($kinrix_SDS = mysqli_fetch_array($sql_run))
                    {
                      echo "<option value='". htmlspecialchars($kinrix_SDS['id']) ."'>" .htmlspecialchars($kinrix_SDS['name']) .' ' .'('.htmlspecialchars($kinrix_SDS['funding_source']).')' ."</option>" ;
                    }
                  ?>
              </option>
            </select>
            ...
            <div class="row mb-2">
                <div class="col">
                  <input type="text" id="add_kinrix_lot" name="lot" class="form-control form-control-sm" value="" placeholder="Lot Number" required>
                </div>
                <div class="col">
                  <input type="text" id="add_kinrix_ndc" name="ndc" class="form-control form-control-sm" value="" placeholder="NDC" required>
                </div>
            </div>
            <?php
            ...
            <div class="row mb-2">
                ...
                <div class="col">
                  <input type="date" id="add_kinrix_exp" name="exp" class="form-control form-control-sm" value="" required>
                </div>
            </div>
            ...
            <div class="row mb-2">
              ...
              <div class="row mb-2">
                ...
                <div class="col">
                  <input id="add_kinrix_funding" name="funding" class="form-control form-control-sm" onChange="add_validate_kinrix()" hidden required>
                  <select id="add_kinrix_eligibility" name="eligibility" class="form-select form-select-sm" onChange="add_validate_kinrix()" required>
                    <option></option>
            ...
            <button type="submit" name="administer_kinrix" class="focus-ring btn btn-sm border mt-3">Submit</button>
          </form>
        </div>
      </div>
    </div>
...
<script>
 function randomNumber(len) {
  ...
  document.getElementById("kinrix_uniqueID").value = randomNumber(8);
</script>
