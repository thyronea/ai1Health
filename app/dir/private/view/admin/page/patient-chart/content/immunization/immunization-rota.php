<div class="tab-pane fade" id="rota" role="tabpanel" tabindex="0">
    <div class="d-flex align-items-start row">
        <div class="col-md-2">
            <button type="button" class="focus-ring btn btn-sm border mt-2 mb-3 shadow" id="submit_btn" data-bs-toggle="modal" data-bs-target="#administer_rota" hidden>Administer Hep B</button> 
            <table class="table table-borderless text-nowrap">
                <tbody align="center" style="text-align: left">
                    <?php
                        $patientID = htmlspecialchars($patient['patientID']);
                        $groupID = mysqli_real_escape_string($con, $_SESSION['groupID']);
                        $vaccine = htmlspecialchars("Rotavirus");
                        ...
                        if($searchnum > 0)
                        {
                            ...
                            {   
                                ...
                                $admin_rota = (date('m', strtotime($admin_date)) . '/' . date('d', strtotime($admin_date)) . '/' . date('Y', strtotime($admin_date))); 
                                ...
                                <tr align="center">
                                    ...
                                    <td >
                                        <a class="text-decoration-none focus-ring edit_rota_btn" style="color: black; font-size: 14px" data-bs-toggle="modal" data-bs-target="#edit_administered_rota">
                                            <div class="card shadow" id="admin_card">
                                                <div class="card-body">
                                                    <small><?=htmlspecialchars($admin_rota);?></small>
                                                </div>
                                            </div>
                                        </a>
                                    </td>
