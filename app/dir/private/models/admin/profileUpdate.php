...
if(isset($_POST['admin_update_profile'])){
  ...
  if($stmt = $con->prepare($contact)){
    ...
    header("Location: /private/view/admin/page/settings/?userID=$userID");
    exit(0);
  }
  ...
  if(mysqli_num_rows($check_diversity_run) > 0)
  {
    ...
    $add_diversity = "INSERT INTO diversity (userID, engineID, groupID, dob, gender, race, ethnicity) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $con->prepare($add_diversity);
    $stmt->bind_param("sssssss", $userID, $engineID, $groupID, $add_encrypt_dob, $gender, $add_encrypt_race. $add_encrypt_ethnicity);
    $stmt->execute();
  }
